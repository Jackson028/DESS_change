...
from models.polarity_contrastive import polarity_contrastive_loss
...
class D2E2SModel(PreTrainedModel):
    ...
    def __init__(
        ...
        self.deberta = AutoModel.from_pretrained(
            args.pretrained_deberta_name, config=config
        )
        ...
        # NEW: Polarity contrastive loss flag
        self.use_polarity_contrastive_loss = getattr(args, "use_polarity_contrastive_loss", False)
    ...
    def _forward_train(
        ...
        # NEW: obtain sentiment logits and representations for contrastive loss
        # chunk processing to reduce memory usage
        senti_repr_list = []
        for i in range(0, sentiments.shape[1], self._max_pairs):
            # classify sentiment candidates
            if self.use_polarity_contrastive_loss:
                chunk_senti_logits, chunk_senti_repr = self._classify_sentiments(
                    entity_spans_pool, size_embeddings, sentiments, senti_masks, h_large, i,
                    return_repr=True,
                )
                senti_repr_list.append(chunk_senti_repr)
            else:
        ...
        # NEW: Compute polarity contrastive loss if enabled
        auxiliary_losses = {}
        if self.use_polarity_contrastive_loss and senti_types is not None:
            senti_repr_full = torch.cat(senti_repr_list, dim=1)  # (B, N, D)
            auxiliary_losses["polarity_contrastive"] = polarity_contrastive_loss(
                senti_repr_full,
                senti_types,
                senti_sample_masks,
                temperature=getattr(self.args, "polarity_contrastive_temperature", 0.07),
            )

        return entity_clf, senti_clf, batch_loss, auxiliary_losses
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings, args):
        ...
        span_h = h.unsqueeze(1).repeat(1, entity_masks.shape[1], 1, 1)
        masked_h = m + span_h

        self.args = args
        if self.args.span_generator == "Average":
            mask_3d_float = entity_masks.float().unsqueeze(-1)
            sum_repr = (span_h * mask_3d_float).sum(dim=2)
            len_repr = mask_3d_float.sum(dim=2).clamp(min=1e-8)
            entity_spans_pool = sum_repr / len_repr
        else:
            entity_spans_pool = masked_h.max(dim=2)[0]
    ...
    def _classify_sentiments(
        ...
        # NEW: return representation if needed for contrastive loss
        if return_repr:
            return chunk_senti_logits, senti_repr
