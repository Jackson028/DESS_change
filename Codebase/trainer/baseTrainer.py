...
class BaseTrainer:
    ...
    def _log_eval(self, ner_prec_micro: float, ner_rec_micro: float, ner_f1_micro: float,
        ...
        # Track best F1 score
        if not hasattr(self, '_best_senti_f1'):
            self._best_senti_f1 = 0.0
            self._best_epoch = 0

        if senti_f1_micro > self._best_senti_f1:
            self._best_senti_f1 = senti_f1_micro
            self._best_epoch = epoch

        # Print current and best F1
        print(f"\n{'='*60}")
        print(f"Epoch {epoch} - Current F1: {senti_f1_micro:.4f} | Best F1: {self._best_senti_f1:.4f} (Epoch {self._best_epoch})")
        print(f"{'='*60}\n")
